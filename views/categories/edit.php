...
$title = 'Edit Category - Helpdesk System';
$pageTitle = 'Edit Category';

// Define icons with their SVG paths (same as create form)
$iconList = [
...
$selectedIcon = $_SESSION['old_input']['icon'] ?? $category['icon'];
...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-6">
        ...
        <p class="mt-1 text-sm text-slate-600">Update category details and settings</p>
    </div>

    <!-- Edit Form -->
    <div class="bg-white shadow rounded-lg">
        <form method="POST" action="/categories/<?= $category['id'] ?>/update" class="p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" 
                       ...
                       name="name" 
                       required
                       value="<?= Security::escape($_SESSION['old_input']['name'] ?? $category['name']) ?>"
            ...
            <div>
                ...
                <textarea id="description" 
                          ...
                          rows="4"
                          class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                          placeholder="Brief description of this category..."><?= Security::escape($_SESSION['old_input']['description'] ?? $category['description']) ?></textarea>
            </div>
            ...
            <div>
                ...
                <div class="flex items-center space-x-4">
                    <input type="color" 
                           id="color" 
                           name="color" 
                           value="<?= Security::escape($_SESSION['old_input']['color'] ?? $category['color']) ?>"
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-xs text-slate-500 mt-1">Current: <span id="color-preview" class="font-mono"><?= strtoupper($category['color']) ?></span></p>
                    </div>
                </div>
            </div>
            ...
            <!-- Category Info -->
            <?php if (isset($category['ticket_count'])): ?>
                ...
                <div class="flex items-start">
                    ...
                    <div class="ml-3">
                        ...
                        <p class="text-sm text-blue-700 mt-1">
                            This category currently has <strong><?= $category['ticket_count'] ?? 0 ?></strong> associated tickets.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-between pt-4 border-t border-slate-200">
                <div>
                    <?php if (($category['ticket_count'] ?? 0) == 0): ?>
                    <button type="button" 
                            onclick="deleteCategory(<?= $category['id'] ?>)"
                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-red-600 hover:text-red-800 hover:bg-red-50 rounded-lg transition">
                        <svg class="mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <button type="button" 
                            disabled
                            class="inline-flex items-center px-4 py-2 text-sm font-medium text-slate-400 cursor-not-allowed rounded-lg"
                            title="Cannot delete category with <?= $category['ticket_count'] ?> tickets">
                        <svg class="mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <button type="submit" 
                            class="inline-flex items-center px-6 py-2 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700 transition">
                        <svg class="mr-2 h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
